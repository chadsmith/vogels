...
Table.prototype.sendRequest = function (method, params, callback) {
  ...
  self.log.info('vogels ' + method.toUpperCase() + ' request', params);
  try {
    driver[method].call(driver, params, function (err, data) {
      var elapsed = Date.now() - startTime;
      self.log.info('requestId: ' + this.requestId + ', elapsed: ' + elapsed + 'ms');
      if (err) {
        self.log.warn('vogels ' + method.toUpperCase() + ' error', err);
        return callback(err);
      } else {
        self.log.info('vogels ' + method.toUpperCase() + ' response', data);
        return callback(null, data);
      }
    });
  }
  catch(e) {
    self.log.error('vogels ' + method.toUpperCase() + ' error', e, driver);
    return callback(e);
  }
};
